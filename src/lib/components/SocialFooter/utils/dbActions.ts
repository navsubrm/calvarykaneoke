...
export function formDataConversion(formData: FormData) {
	const componentDBDataObject: App.Components = {
		...
		content: JSON.stringify({
			...
			x_post_text: formData?.x_post_text.toString(),
			facebook_share_link: formData?.facebook_share_link.toString(),
			mail_to: {
				subject: formData?.mail_to_subject.toString(),
				body: formData?.mail_to_body.toString()
			}
		})
	};
