<script lang="ts">
	...
	const pageId: string = $state('contact');
	const pageRoute: string = $state('/contact');
...
<svelte:head>
	...
	<title>Contact Page Editor</title>
</svelte:head>
	...
	{:else}
		...
		>
			...
			<details>
				...
				<fieldset>
					<legend>Aside Content</legend>

					<div class="editor">
						<QuillInput
							name="aside_content"
							label="Provide Aside Content"
							placeholder="Enter the content you want displayed next to the contact form."
							themeBase={'One'}
							bind:value={pageData.content.aside_content.main}
						/>
					</div>
				</fieldset>
			</details>
