<script lang="ts">
	 ...
	 * */
	import blank from '$lib/config/dataModels/FrequentlyRequestedPdfs';
	const pageId: string = $state('frequently-requested-pdfs');
	const pageRoute: string = $state('/frequently-requested-pdfs');
...
<svelte:head>
	...
	<title>Frequently Requested PDFs Page Editor</title>
</svelte:head>

<section style="--_background: var({theme.one}); --_text: var({theme.two})">
	<div class="page-header">
		<h1>Frequently Requested PDFs Page Editor</h1>
			...
			<details>
				...
				<fieldset>
					...
					<div class="editor">
						<QuillInput
							name="main_content"
							label="Provide Page Content"
							placeholder="Enter the page content."
							themeBase={'One'}
							bind:value={pageData.content.main_content.main}
						/>
					</div>
				</fieldset>
			</details>
