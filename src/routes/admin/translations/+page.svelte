<script lang="ts">
	...
	const pageId: string = $state('translations');
	const pageRoute: string = $state('/translations');
	const pageTitle: string = $state('Translations Page Editor');
...
<section style="--_background: var({theme.one}); --_text: var({theme.two})">
	...
	{:else}
		...
		>
			...
			<details>
				<summary>Translation Link Data</summary>
